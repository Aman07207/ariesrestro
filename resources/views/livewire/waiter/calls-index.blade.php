<div>
    ...
    @else
        @foreach($calls as $call)
            <div class="card" style="margin-bottom:10px; display:flex; justify-content:space-between; align-items:center;" wire:key="call-{{ $call->id }}">
                <div>
                ...
                @if($call->status === \App\Enums\WaiterCallStatus::Pending)
                    <button type="button" class="btn btn-sm btn-dark" wire:click="attend({{ $call->id }})" wire:loading.attr="disabled">Mark attended</button>
                @else
